...
def get_interface_codes(
    ...
    for interface_name, interface_path in interface_codes.items():
        ...
        if key := next(
            (f"{i}.vy" for i in keys if f"{i}.vy" in contract_sources), None
        ):
...
def format_to_output_dict(compiler_data: Dict) -> Dict:
    ...
    for id_, (path, data) in enumerate(compiler_data.items()):
        ...
        if (
            any(f"{i}_runtime" in data for i in evm_keys)
            or "source_map" in data
        ):
    ...
    try:
        ...
        if warn_data:
            output_dict["errors"] = [
                ...
                for path, msg in (
                    (k, x) for k, v in warn_data.items() for x in v
                )
            ]
        return output_dict
